...
| #620  | SAS URLs broader than one read-only, short-lived blob under the caller's prefix      | Unit on the adapter + integration  | Not       | Handler unit tests fake `generateSasUrl`; nothing parses a real URL's `sp`, `sr` or expiry                                                                                             |
...
| #564  | Vendored MSAL and Leaflet get no update PRs                                          | Supply-chain automation            | Not       | Dependabot npm covers `/functions` only; `frontend/src/vendor/.msal-source` pins 3.28.1 by hand                                                                                        |
...
| #551  | `GET /api/me` overwrites the user's chosen display name with the token claim         | Handler unit + full-stack E2E      | Not       | A `GetMe` unit test pins the overwrite; the E2E rename journey never reloads                                                                                                           |
...
| #617  | Deletion job has no dry-run mode                                                     | Unit on the job + dry run          | Not       | `workflow_dispatch` has no dry-run input                                                                                                                                               |
...
| #618  | Integration cleanup trusts `COSMOS_CONNECTION_STRING`                                | Integration setup guard            | Not       | `afterAll` deletes touch only their own random ids, but nothing refuses a non-emulator endpoint                                                                                        |
