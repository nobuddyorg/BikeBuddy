...
| -------------------------------------------------------------------------- | ---------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Token validation (issuer, audience, alg, expiry, key id)                   | Middleware unit, test-signed             | `functions/src/middleware/authMiddleware.test.js`; `test/unit/tokens.test.js` runs every harness token through `authenticate()`; `test/unit/localIssuer.test.js` does it over HTTP against the local issuer                             |
...
| #557  | Frontend does not handle 401 / expired tokens                                        | DOM-layer unit                     | Not       | `frontend/src/ui/` has unit tests only for the i18n runtime, line-style storage and the upload request; nothing covers a 401                                                                                            |
...
| #543  | Stateful resources lack destroy guards under `tofu apply -auto-approve`              | IaC scan                           | Covered   | `prevent_destroy` on the resource group, Cosmos account, database, three containers, storage account and `gpx-files`; TFLint's rule is on, with two per-resource exceptions (`images`, `deployments`)                   |
...
| #538  | Account deletion can orphan data (queue keyed by oid, `GetMe` recreates the user)    | Integration                        | Partly    | `account-deletion.test.js`: the user's list, export and signed URLs are empty or 404 and another user is untouched; re-creation by `GetMe` and the queue write (integration tokens carry no `oid`) are unit-tested only |
...
| #577  | No schema versioning; backfill scripts lack dry run and batching                     | Handler unit (old shape) + dry run | Partly    | Backfills are dry by default, paged and unit-tested; old documents are still recognised by a missing field, no schema version                                                                                           |
...
- **The auth rules without a test**: an ID token presented as an access token
  is accepted, so it has no rejection case in the matrix (#569); `SKIP_AUTH`
  at deploy time (#545).
