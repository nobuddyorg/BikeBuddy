...
| Mutation scope + floor | Stryker             | 40 functions modules (incl. `scripts/lib/`, the integration guard and token minter), 21 frontend modules; the same list sets the 100 % per-file floor                       | `mutation-targets.mjs`                                                            |
...
| #537  | Unauthenticated requests drain the JWKS rate limiter → 500 for everyone              | Middleware unit (injected JWKS)    | Not       | `rateLimit: true` on the shared jwks-rsa client; the integration suite sends an unknown-kid token only once for this reason                                                                                             |
...
- **The auth rules without a test**: an ID token presented as an access token
  is accepted, so it has no rejection case in the matrix (#569); JWKS drain
  (#537), timeouts and key rotation (#571), `SKIP_AUTH` at deploy time (#545).
