...
// #230: the sidebar list paginates at 15 tours/page. Seeds 25 tours directly
...
buddyTest.describe('tour list pagination', () => {
  ...
  buddyTest('pages through 25 tours, 15 per page', async ({ on, page }) => {
    await page.goto('/');
    await expect(on(page).main.locators.userMenu).toBeVisible();
    await expect(on(page).main.locators.list.count).toHaveText('25');

    await expect(on(page).main.locators.list.names).toHaveCount(15);
