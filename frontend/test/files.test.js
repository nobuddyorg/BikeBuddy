...
describe('isGpxFile', () => {
  it('accepts .gpx regardless of case', () => {
    expect(isGpxFile(file({ name: 'ride.gpx' }))).toBe(true);
    expect(isGpxFile(file({ name: 'RIDE.GPX' }))).toBe(true);
  });

  it('rejects other extensions and missing files', () => {
    expect(isGpxFile(file({ name: 'ride.txt' }))).toBe(false);
...
describe('validateGpxUpload', () => {
  it('accepts a valid file', () => {
    expect(validateGpxUpload(file({ name: 'ride.gpx', type: '', size: 1000 }))).toEqual([]);
  });

  it('reports the wrong extension', () => {
    expect(validateGpxUpload(file({ name: 'ride.txt' }))).toEqual([
      { key: 'errors.gpxType', params: {} },
    ]);
  });

  it('reports a file over the size limit, with the limit in megabytes', () => {
    expect(
      validateGpxUpload(file({ name: 'ride.gpx', type: '', size: MAX_UPLOAD_BYTES + 1 })),
    ).toEqual([{ key: 'errors.gpxSize', params: { maxMegabytes: 10 } }]);
  });

  it('accepts a file exactly at the size limit', () => {
    expect(validateGpxUpload(file({ name: 'ride.gpx', type: '', size: MAX_UPLOAD_BYTES }))).toEqual(
      [],
    );
  });
});
...
describe('validateImageUpload', () => {
  it('accepts a valid image', () => {
    expect(validateImageUpload(file({ name: 'p.jpg', type: 'image/jpeg', size: 1000 }))).toEqual(
      [],
    );
  });

  it('reports non-images and oversized images', () => {
    expect(validateImageUpload(file({ name: 'p.gif', type: 'image/gif' }))).toEqual([
      { key: 'errors.imageType', params: {} },
    ]);
    expect(
      validateImageUpload(file({ name: 'p.png', type: 'image/png', size: 10 * 1024 * 1024 + 1 })),
    ).toEqual([{ key: 'errors.imageSize', params: { maxMegabytes: 10 } }]);
  });

  it('accepts an image exactly at the size limit', () => {
    expect(
      validateImageUpload(file({ name: 'p.png', type: 'image/png', size: 10 * 1024 * 1024 })),
    ).toEqual([]);
  });
});
...
describe('validateImageBatch', () => {
  it('accepts a batch at the cap', () => {
    const files = Array.from({ length: MAX_IMAGE_BATCH }, (_, i) => file({ name: `p${i}.jpg` }));
    expect(validateImageBatch(files)).toEqual([]);
  });

  it('reports a batch over the cap', () => {
    const files = Array.from({ length: MAX_IMAGE_BATCH + 1 }, (_, i) =>
      file({ name: `p${i}.jpg` }),
    );
    expect(validateImageBatch(files)).toEqual([
...
describe('planImageUploads', () => {
  const jpeg = (name) => file({ name, type: 'image/jpeg' });
  ...
  it('stops accepting once the tour is full, counting only accepted photos', () => {
    const files = [file({ name: 'bad.gif', type: 'image/gif' }), jpeg('a.jpg'), jpeg('b.jpg')];
  ...
  it('lists the quota problem before the file problem', () => {
    const [{ problems }] = planImageUploads({
      files: [file({ name: 'bad.gif', type: 'image/gif' })],
